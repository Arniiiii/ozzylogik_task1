...
It's a test task for "Ozzylogik" company.

Task was: 
 using either Qt or boost
...
The standalone that can be got using next commands:
```
cmake -S ./standalone -B ./build
cmake --build ./build --target Task1Standalone --parallel $(nproc)
./build/standalone/Task1
```
